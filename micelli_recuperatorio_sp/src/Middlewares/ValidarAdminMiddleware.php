...
//Valida que se envie un token de autenticacion y que ademas sea de tipo admin
class ValidarAdminMiddleware
{
    ...
    public function __invoke(Request $request, RequestHandler $handler): Response
    {
        ...
        if($tokenRecibido == true)
        {
            ...
            if(ValidarAdminMiddleware::validarTokenJWT($token))
            {
                ...
                else
                {
                    ...
                    $response->getBody()->write(json_encode("Solo un admin puede registrar una mascota o marcar como atendido un turno"));
                }
            }
            else
            {
        ...
        {
            $response->withStatus(403);
            $response->getBody()->write(json_encode("Para registrar una mascota o marcar un turno como atendido debe enviar token con tipo admin por headers"));
        }
