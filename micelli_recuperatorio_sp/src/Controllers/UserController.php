...
class UserController
{
    ...
    {
        //localhost/micelli_segundo_parcial/public/user/5
    ...
    {       
        ...
        if(!(empty($parsedBody["email"])) && !(empty($parsedBody["tipo"])) && !(empty($parsedBody["clave"])) && !(empty($parsedBody["nombre"])))
        {
            ...
            if($validacionMail == true)
            {
                ...
                {
                    $response->getBody()->write(json_encode("Error: tipo solo puede ser admin o cliente"));
                }
            }
            else
            {
        ...
        if(!(empty($parsedBody["email"])) || !(empty($parsedBody["nombre"])))
        {
            ...
            {   
                ...
                if(!empty($usuarioDecodifcado))
                {
                    ...
                    $token = UserController::generarTokenJWT($email, $tipo, $id, $nombre);
    ...
    public static function generarTokenJWT($email, $tipo, $id, $nombre)
    {
        $key = "segundoparcial";
